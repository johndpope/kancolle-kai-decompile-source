...
public class iTweenEvent : MonoBehaviour
{
	...
	public void Stop()
	{
		iTween.StopByName(base.get_gameObject(), this.internalName);
	...
	public void OnDrawGizmos()
	{
		...
		{
			Gizmos.DrawIcon(base.get_transform().get_position(), "iTweenIcon.tif");
		}
	}
	...
	{
		iTweenEvent.<StartEvent>c__Iterator9 <StartEvent>c__Iterator = new iTweenEvent.<StartEvent>c__Iterator9();
		<StartEvent>c__Iterator.<>f__this = this;
		return <StartEvent>c__Iterator;
	}
	...
	private void SerializeValues()
	{
		...
		using (Dictionary<string, object>.Enumerator enumerator = this.values.GetEnumerator())
		{
			while (enumerator.MoveNext())
			{
				KeyValuePair<string, object> current = enumerator.get_Current();
				Dictionary<string, Type> dictionary = EventParamMappings.mappings.get_Item(this.type);
				Type type = dictionary.get_Item(current.get_Key());
				if (typeof(int) == type)
				{
				...
				{
					if (current.get_Value() == null || typeof(Transform) == current.get_Value().GetType())
					{
						this.AddToList<Transform>(list, list2, list14, list3, current.get_Key(), current.get_Value(), "t");
					}
					else
					{
						this.AddToList<Vector3>(list, list2, list8, list3, current.get_Key(), current.get_Value(), "v");
					}
				}
				else if (typeof(Vector3OrTransformArray) == type)
				{
					if (typeof(Vector3[]) == current.get_Value().GetType())
					{
						Vector3[] array = (Vector3[])current.get_Value();
						ArrayIndexes arrayIndexes = new ArrayIndexes();
						int[] array2 = new int[array.Length];
						for (int i = 0; i < array.Length; i++)
						{
							list8.Add(array[i]);
							array2[i] = list8.get_Count() - 1;
						}
						arrayIndexes.indexes = array2;
						this.AddToList<ArrayIndexes>(list, list2, list17, list3, current.get_Key(), arrayIndexes, "v");
					}
					else if (typeof(Transform[]) == current.get_Value().GetType())
					{
						Transform[] array3 = (Transform[])current.get_Value();
						ArrayIndexes arrayIndexes2 = new ArrayIndexes();
						int[] array4 = new int[array3.Length];
						for (int j = 0; j < array3.Length; j++)
						{
							list14.Add(array3[j]);
							array4[j] = list14.get_Count() - 1;
						}
						arrayIndexes2.indexes = array4;
						this.AddToList<ArrayIndexes>(list, list2, list18, list3, current.get_Key(), arrayIndexes2, "t");
					}
					else if (typeof(string) == current.get_Value().GetType())
					{
						this.AddToList<string>(list, list2, list7, list3, current.get_Key(), current.get_Value(), "p");
					}
				}
			}
		}
	...
	{
		this.AddToList<T>(keyList, indexList, valueList, metadataList, pair.get_Key(), pair.get_Value());
	}
	...
	{
		this.AddToList<T>(keyList, indexList, valueList, metadataList, pair.get_Key(), pair.get_Value(), metadata);
	}
	...
	private void AddToList<T>(List<string> keyList, List<int> indexList, IList<T> valueList, List<string> metadataList, string key, object value, string metadata)
	{
		...
		indexList.Add(valueList.get_Count() - 1);
	...
	private void DeserializeValues()
	{
		...
		{
			Dictionary<string, Type> dictionary = EventParamMappings.mappings.get_Item(this.type);
			Type type = dictionary.get_Item(this.keys[i]);
			if (typeof(int) == type)
			{
	...
	private void init()
	{
		if (this.myPosition == new Vector3(0f, 0f, 0f))
		{
			this.myPosition = new Vector3(base.get_transform().get_localPosition().x, base.get_transform().get_localPosition().y, base.get_transform().get_localPosition().z);
		}
		base.get_gameObject().get_transform().set_localPosition(new Vector3(this.myPosition.x, this.myPosition.y, this.myPosition.z));
	}
}
