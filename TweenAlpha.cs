...
public class TweenAlpha : UITweener
{
	...
	public float value
	{
		get
		{
			...
			if (this.mSr != null)
			{
				return this.mSr.get_color().a;
			}
			return (!(this.mMat != null)) ? 1f : this.mMat.get_color().a;
		}
		set
		{
			if (!this.mCached)
			...
			{
				Color color = this.mSr.get_color();
				color.a = value;
				this.mSr.set_color(color);
			}
			else if (this.mMat != null)
			{
				Color color2 = this.mMat.get_color();
				color2.a = value;
				this.mMat.set_color(color2);
			}
		}
	}
