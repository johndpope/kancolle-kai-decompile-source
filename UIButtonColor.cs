...
public class UIButtonColor : UIWidgetContainer
{
	...
	public Color hover = Util.CursolColor;
	...
	[NonSerialized]
	public bool OnEnableAndOnDisableChangeState;
	...
	protected virtual void OnInit()
	{
		...
		else if (this.tweenTarget != null)
		{
			...
			if (component != null)
			{
				this.mDefaultColor = ((!Application.get_isPlaying()) ? component.get_sharedMaterial().get_color() : component.get_material().get_color());
				this.mStartingColor = this.mDefaultColor;
			}
			else
			{
				...
				if (component2 != null)
				{
					this.mDefaultColor = component2.get_color();
					this.mStartingColor = this.mDefaultColor;
				}
				else
				{
	...
	protected virtual void OnEnable()
	{
		...
		{
			if (UICamera.currentTouch.pressed == base.get_gameObject())
			{
				this.OnPress(true);
			}
			else if (UICamera.currentTouch.current == base.get_gameObject())
			{
				this.OnHover(true);
			}
		}
		...
		if (this.isEnabled && UICamera.currentTouch != null)
		{
			...
			if (this.tweenTarget != null)
			{
				...
				else if (UICamera.currentTouch.current == base.get_gameObject())
				{
					...
					else if (UICamera.currentScheme == UICamera.ControlScheme.Mouse && UICamera.hoveredObject == base.get_gameObject())
					{
						this.SetState(UIButtonColor.State.Hover, false);
					}
					else
	...
	{
		if (this.isEnabled && !this.mInitDone)
		{
			this.OnInit();
		}
	}
