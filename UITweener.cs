...
public abstract class UITweener : MonoBehaviour
{
	...
	private void Update()
	{
		float num = (!this.ignoreTimeScale) ? Time.get_deltaTime() : RealTime.deltaTime;
		float num2 = (!this.ignoreTimeScale) ? Time.get_time() : RealTime.time;
		if (!this.mStarted)
		{
			this.mStarted = true;
		...
		if (this.style == UITweener.Style.Once && (this.duration == 0f || this.mFactor > 1f || this.mFactor < 0f))
		{
			...
			if (UITweener.current == null)
			{
				...
				if (this.onFinished != null)
				{
					...
					for (int i = 0; i < this.mTemp.get_Count(); i++)
					{
						EventDelegate eventDelegate = this.mTemp.get_Item(i);
