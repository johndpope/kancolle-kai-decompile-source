...
public class BMGlyph
{
	...
	public int GetKerning(int previousChar)
	{
		...
		{
			int i = 0;
			int count = this.kerning.get_Count();
			while (i < count)
			{
				if (this.kerning.get_Item(i) == previousChar)
				{
					return this.kerning.get_Item(i + 1);
				}
				i += 2;
			}
		}
	...
	public void SetKerning(int previousChar, int amount)
	{
		...
		for (int i = 0; i < this.kerning.get_Count(); i += 2)
		{
			if (this.kerning.get_Item(i) == previousChar)
			{
				this.kerning.set_Item(i + 1, amount);
				return;
			}
		}
