...
public class EventDelegate
{
	...
	public class Parameter
	{
		...
		public object value
		{
			get
			{
				...
				if (this.expectedType != null && this.expectedType.get_IsValueType())
				{
					return null;
				}
	...
	public bool isEnabled
	{
		get
		{
			...
			return monoBehaviour == null || monoBehaviour.get_enabled();
		}
	}
	...
	private void Cache()
	{
		...
		if ((this.mCachedCallback == null || this.mCachedCallback.get_Target() as MonoBehaviour != this.mTarget || EventDelegate.GetMethodName(this.mCachedCallback) != this.mMethodName) && this.mTarget != null && !string.IsNullOrEmpty(this.mMethodName))
		{
			...
			if (this.mMethod.get_ReturnType() != typeof(void))
			{
				Debug.LogError(string.Concat(new object[]
				{
					this.mTarget.GetType(),
			...
			while (j < num2)
			{
				this.mParameters[j].expectedType = this.mParameterInfos[j].get_ParameterType();
				j++;
			}
		}
	}
			...
			else
			{
				...
				catch (ArgumentException ex)
				{
					...
					else
					{
						...
						{
							text = text + ", " + this.mParameterInfos[j].get_ParameterType();
						}
					}
					text += "\n  Received: ";
				...
				while (l < num2)
				{
					if (this.mParameterInfos[l].get_IsIn() || this.mParameterInfos[l].get_IsOut())
					{
		...
		if (list != null)
		{
			for (int i = 0; i < list.get_Count(); i++)
			{
				...
				if (eventDelegate != null)
				{
					...
					if (list.get_Item(i) != eventDelegate)
					{
						continue;
					}
