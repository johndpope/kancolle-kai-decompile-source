...
public abstract class UIRect : MonoBehaviour
{
	...
	public class AnchorPoint
	{
		...
		public void SetHorizontal(Transform parent, float localPos)
		{
			...
			{
				Vector3 vector = this.target.get_position();
				if (parent != null)
				{
	...
	public UIRect parent
	{
		get
		{
			if (!this.mParentFound)
			{
				...
				this.mParent = NGUITools.FindInParents<UIRect>(this.cachedTransform.get_parent());
			}
			return this.mParent;
		}
	}
	...
	protected float cameraRayDistance
	{
		get
		{
			...
			if (!this.mCam.get_orthographic())
			{
				Transform cachedTransform = this.cachedTransform;
				Transform transform = this.mCam.get_transform();
				Plane plane = new Plane(cachedTransform.get_rotation() * Vector3.get_back(), cachedTransform.get_position());
				Ray ray = new Ray(transform.get_position(), transform.get_rotation() * Vector3.get_forward());
				float result;
			...
			return Mathf.Lerp(this.mCam.get_nearClipPlane(), this.mCam.get_farClipPlane(), 0.5f);
		}
	}
	...
	protected Vector3 GetLocalPos(UIRect.AnchorPoint ac, Transform trans)
	{
		...
		{
			return this.cachedTransform.get_localPosition();
		}
		Rect rect = ac.targetCam.get_rect();
		Vector3 vector = ac.targetCam.WorldToViewportPoint(ac.target.get_position());
		Vector3 vector2 = new Vector3(vector.x * rect.get_width() + rect.get_x(), vector.y * rect.get_height() + rect.get_y(), vector.z);
	...
	public void ResetAnchors()
	{
		...
		this.mCam = NGUITools.FindCameraForLayer(this.cachedGameObject.get_layer());
	...
	{
		this.mParentFound = false;
		UIRect uIRect = NGUITools.FindInParents<UIRect>(this.cachedTransform.get_parent());
