...
public class UIToggle : UIWidgetContainer
{
	...
	public List<EventDelegate> onActive = new List<EventDelegate>();
	...
	private void Start()
	{
		...
		if (!Application.get_isPlaying())
		{
			...
			if (Application.get_isPlaying() && this.activeSprite != null)
			{
	...
	private void OnClick()
	{
		if (base.get_enabled())
		{
			if (this.value && this.onDecide != null)
			{
				this.onDecide.Invoke();
			}
	...
	public void Set(bool state)
	{
		...
		else if (this.mIsActive != state)
		{
			...
			if (this.activeSprite != null)
			{
				...
				{
					TweenAlpha.Begin(this.activeSprite.get_gameObject(), 0.15f, (!this.mIsActive) ? 0f : 1f);
				}
			}
			if (UIToggle.current == null)
			{
				...
				if (this.value && EventDelegate.IsValid(this.onActive))
				{
					EventDelegate.Execute(this.onActive);
				}
