...
public class UIDraggableCamera : MonoBehaviour
{
	...
	public bool ConstrainToBounds(bool immediate)
	{
		if (this.mTrans != null && this.rootForBounds != null)
		{
			...
			if (vector.get_sqrMagnitude() > 0f)
			{
				if (immediate)
				{
					Transform expr_46 = this.mTrans;
					expr_46.set_position(expr_46.get_position() - vector);
				}
				else
				{
					SpringPosition springPosition = SpringPosition.Begin(base.get_gameObject(), this.mTrans.get_position() - vector, 13f);
	...
	public void Drag(Vector2 delta)
	{
		...
		Transform expr_65 = this.mTrans;
		expr_65.set_localPosition(expr_65.get_localPosition() + vector);
