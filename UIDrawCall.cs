...
public class UIDrawCall : MonoBehaviour
{
	...
	public int renderQueue
	{
		...
		set
		{
			if (this.mRenderQueue != value)
			{
				...
				{
					this.mDynamicMat.set_renderQueue(value);
				}
			}
		}
	}
	...
	public int sortingOrder
	{
		get
		{
			return (!(this.mRenderer != null)) ? 0 : this.mRenderer.get_sortingOrder();
		}
		set
		{
			if (this.mRenderer != null && this.mRenderer.get_sortingOrder() != value)
			{
				this.mRenderer.set_sortingOrder(value);
			}
		}
	}
	...
	public int finalRenderQueue
	{
		get
		{
			return (!(this.mDynamicMat != null)) ? this.mRenderQueue : this.mDynamicMat.get_renderQueue();
		}
	}
	...
	public Texture mainTexture
	{
		...
		set
		{
			...
			if (this.mDynamicMat != null)
			{
				this.mDynamicMat.set_mainTexture(value);
			}
		}
	}
	...
	private void CreateMaterial()
	{
		...
		string text = (!(this.mShader != null)) ? ((!(this.mMaterial != null)) ? "Unlit/Transparent Colored" : this.mMaterial.get_shader().get_name()) : this.mShader.get_name();
		...
		if (this.mMaterial != null)
		{
			...
			this.mDynamicMat.set_name("[NGUI] " + this.mMaterial.get_name());
			this.mDynamicMat.set_hideFlags(60);
			this.mDynamicMat.CopyPropertiesFromMaterial(this.mMaterial);
			string[] shaderKeywords = this.mMaterial.get_shaderKeywords();
			...
			if (this.shader != null)
			{
				this.mDynamicMat.set_shader(this.shader);
			}
			else if (this.mClipCount != 0)
			{
		...
		else
		{
			...
			this.mDynamicMat.set_name("[NGUI] " + this.shader.get_name());
			this.mDynamicMat.set_hideFlags(60);
		}
	}
	...
	private Material RebuildMaterial()
	{
		...
		this.CreateMaterial();
		this.mDynamicMat.set_renderQueue(this.mRenderQueue);
		if (this.mTexture != null)
		{
			this.mDynamicMat.set_mainTexture(this.mTexture);
		}
		if (this.mRenderer != null)
		{
			this.mRenderer.set_sharedMaterials(new Material[]
			{
				this.mDynamicMat
			});
		}
	...
	private void UpdateMaterials()
	{
		...
		else if (this.mRenderer.get_sharedMaterial() != this.mDynamicMat)
		{
			this.mRenderer.set_sharedMaterials(new Material[]
			{
				this.mDynamicMat
	...
	public void UpdateGeometry(int widgetCount)
	{
		...
		if (size > 0 && size == this.uvs.size && size == this.cols.size && size % 4 == 0)
		{
			...
			{
				this.mFilter = base.get_gameObject().GetComponent<MeshFilter>();
			}
			if (this.mFilter == null)
			{
				this.mFilter = base.get_gameObject().AddComponent<MeshFilter>();
			}
			if (this.verts.size < 65000)
			{
				...
				{
					this.mMesh = new Mesh();
					this.mMesh.set_hideFlags(52);
					this.mMesh.set_name((!(this.mMaterial != null)) ? "[NGUI] Mesh" : ("[NGUI] " + this.mMaterial.get_name()));
				...
				{
					flag2 = (this.mMesh == null || this.mMesh.get_vertexCount() != this.verts.buffer.Length);
				}
				...
				{
					if (flag2 || this.mMesh.get_vertexCount() != this.verts.size)
					{
						this.mMesh.Clear();
						flag = true;
					}
					this.mMesh.set_vertices(this.verts.ToArray());
					this.mMesh.set_uv(this.uvs.ToArray());
					this.mMesh.set_colors32(this.cols.ToArray());
					if (this.norms != null)
					{
						this.mMesh.set_normals(this.norms.ToArray());
					}
					if (this.tans != null)
					{
						this.mMesh.set_tangents(this.tans.ToArray());
					}
				}
				else
				{
					if (this.mMesh.get_vertexCount() != this.verts.buffer.Length)
					{
						this.mMesh.Clear();
						flag = true;
					}
					this.mMesh.set_vertices(this.verts.buffer);
					this.mMesh.set_uv(this.uvs.buffer);
					this.mMesh.set_colors32(this.cols.buffer);
					if (this.norms != null)
					{
						this.mMesh.set_normals(this.norms.buffer);
					}
					if (this.tans != null)
					{
						this.mMesh.set_tangents(this.tans.buffer);
					}
				}
				if (flag)
				{
					this.mIndices = this.GenerateCachedIndexBuffer(size, num);
					this.mMesh.set_triangles(this.mIndices);
				}
				if (flag2 || !this.alwaysOnScreen)
				{
					this.mMesh.RecalculateBounds();
				}
				this.mFilter.set_mesh(this.mMesh);
			}
			else
			{
				this.mTriangles = 0;
				if (this.mFilter.get_mesh() != null)
				{
					this.mFilter.get_mesh().Clear();
				}
				Debug.LogError("Too many vertices on one panel: " + this.verts.size);
			}
			if (this.mRenderer == null)
			{
				this.mRenderer = base.get_gameObject().GetComponent<MeshRenderer>();
			}
			if (this.mRenderer == null)
			{
				this.mRenderer = base.get_gameObject().AddComponent<MeshRenderer>();
			}
	...
	{
		int i = 0;
		int count = UIDrawCall.mCache.get_Count();
		while (i < count)
		{
			int[] array = UIDrawCall.mCache.get_Item(i);
		...
		if (UIDrawCall.mCache.get_Count() > 10)
		{
