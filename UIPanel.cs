...
public class UIPanel : UIRect
{
	...
	public static int nextUnusedDepth
	{
		get
		{
			...
			while (i < count)
			{
				num = Mathf.Max(num, UIPanel.list.get_Item(i).depth);
				i++;
			}
	...
	public bool usedForUI
	{
		get
		{
			return base.anchorCamera != null && this.mCam.get_orthographic();
		}
	}
	...
	public Vector3 drawCallOffset
	{
		get
		{
			if (this.mHalfPixelOffset && base.anchorCamera != null && this.mCam.get_orthographic())
			{
				Vector2 windowSize = this.GetWindowSize();
				float num = (!(base.root != null)) ? 1f : base.root.pixelSizeAdjustment;
				float num2 = num / windowSize.y / this.mCam.get_orthographicSize();
				return new Vector3(-num2, num2);
			}
	...
	public override Vector3[] worldCorners
	{
		get
		{
			...
			else
			{
				...
				if (this.anchorOffset && (this.mCam == null || this.mCam.get_transform().get_parent() != base.cachedTransform))
				{
	...
	public void SetDirty()
	{
		...
		while (i < count)
		{
			this.drawCalls.get_Item(i).isDirty = true;
			i++;
		}
	...
	private void Awake()
	{
		this.mGo = base.get_gameObject();
		this.mTrans = base.get_transform();
		this.mHalfPixelOffset = (Application.get_platform() == 2 || Application.get_platform() == 10 || Application.get_platform() == 5 || Application.get_platform() == 7);
		if (this.mHalfPixelOffset)
		{
			this.mHalfPixelOffset = (SystemInfo.get_graphicsShaderLevel() < 40 && SystemInfo.get_graphicsDeviceVersion().Contains("Direct3D"));
		}
	}
	...
	private void FindParent()
	{
		...
		this.mParentPanel = ((!(parent != null)) ? null : NGUITools.FindInParents<UIPanel>(parent.get_gameObject()));
	}
	...
	{
		this.mLayer = this.mGo.get_layer();
	}
	...
	protected override void OnInit()
	{
		...
		if (base.GetComponent<Rigidbody>() == null && this.mParentPanel == null)
		{
			...
			{
				Rigidbody rigidbody = base.get_gameObject().AddComponent<Rigidbody>();
	...
	private void UpdateTransformMatrix()
	{
		...
		if (this.mMatrixFrame != frameCount)
		{
			...
			this.worldToLocal = base.cachedTransform.get_worldToLocalMatrix();
	...
	protected override void OnAnchor()
	{
		...
		Vector2 viewSize = this.GetViewSize();
		Vector2 vector = cachedTransform.get_localPosition();
		float num;
		float num2;
		float num3;
		float num4;
	...
	{
		if (UIPanel.mUpdateFrame != Time.get_frameCount())
		{
			UIPanel.mUpdateFrame = Time.get_frameCount();
			int i = 0;
			int count = UIPanel.list.get_Count();
			while (i < count)
			{
				UIPanel.list.get_Item(i).UpdateSelf();
				i++;
			}
			int num = 3000;
			int j = 0;
			int count2 = UIPanel.list.get_Count();
			while (j < count2)
			{
				UIPanel uIPanel = UIPanel.list.get_Item(j);
				if (uIPanel.renderQueue == UIPanel.RenderQueue.Automatic)
				{
					uIPanel.startingRenderQueue = num;
					uIPanel.UpdateDrawCalls();
					num += uIPanel.drawCalls.get_Count();
				}
				else if (uIPanel.renderQueue == UIPanel.RenderQueue.StartAt)
				{
					uIPanel.UpdateDrawCalls();
					if (uIPanel.drawCalls.get_Count() != 0)
					{
						num = Mathf.Max(num, uIPanel.startingRenderQueue + uIPanel.drawCalls.get_Count());
					}
				}
				else
				{
	...
	private void UpdateSelf()
	{
		...
		{
			int i = 0;
			while (i < this.drawCalls.get_Count())
			{
				UIDrawCall uIDrawCall = this.drawCalls.get_Item(i);
	...
	private void UpdateDrawCalls()
	{
		...
		{
			this.drawCallClipRange = Vector4.get_zero();
		}
		if (this.drawCallClipRange.z == 0f)
		{
			this.drawCallClipRange.z = (float)Screen.get_width() * 0.5f;
		}
		if (this.drawCallClipRange.w == 0f)
		{
			this.drawCallClipRange.w = (float)Screen.get_height() * 0.5f;
		}
		...
		{
			Transform parent = base.cachedTransform.get_parent();
			vector = base.cachedTransform.get_localPosition();
			if (this.clipping != UIDrawCall.Clipping.None)
			{
		...
		{
			vector = cachedTransform.get_position();
		}
		...
		for (int i = 0; i < this.drawCalls.get_Count(); i++)
		{
			...
			cachedTransform2.set_position(vector);
	...
	{
		if (this.mLayer != base.cachedGameObject.get_layer())
		{
			this.mLayer = this.mGo.get_layer();
			int i = 0;
			int count = this.widgets.get_Count();
			while (i < count)
			{
				UIWidget uIWidget = this.widgets.get_Item(i);
				if (uIWidget && uIWidget.parent == this)
				{
					uIWidget.get_gameObject().set_layer(this.mLayer);
				}
				i++;
			}
			base.ResetAnchors();
			for (int j = 0; j < this.drawCalls.get_Count(); j++)
			{
				this.drawCalls.get_Item(j).get_gameObject().set_layer(this.mLayer);
			}
		}
	}
	...
	public UIDrawCall FindDrawCall(UIWidget w)
	{
		...
		{
			UIDrawCall uIDrawCall = this.drawCalls.get_Item(i);
			int num = (i != 0) ? (this.drawCalls.get_Item(i - 1).depthEnd + 1) : -2147483648;
			int num2 = (i + 1 != this.drawCalls.get_Count()) ? (this.drawCalls.get_Item(i + 1).depthStart - 1) : 2147483647;
	...
	public void AddWidget(UIWidget w)
	{
		...
		else if (UIWidget.PanelCompareFunc(w, this.widgets.get_Item(0)) == -1)
		{
			this.widgets.Insert(0, w);
		}
		else
		{
			int i = this.widgets.get_Count();
			while (i > 0)
			{
				if (UIWidget.PanelCompareFunc(w, this.widgets.get_Item(--i)) != -1)
				{
	...
	public void Refresh()
	{
		...
		if (UIPanel.list.get_Count() > 0)
		{
			UIPanel.list.get_Item(0).LateUpdate();
		}
	}
