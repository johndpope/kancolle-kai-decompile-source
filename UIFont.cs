...
public class UIFont : MonoBehaviour
{
	...
	public bool hasSymbols
	{
		get
		{
			return (!(this.mReplacement != null)) ? (this.mSymbols != null && this.mSymbols.get_Count() != 0) : this.mReplacement.hasSymbols;
		}
	}
	...
	public Material material
	{
		get
		{
			...
			if (this.mMat != null)
			{
				if (this.mDynamicFont != null && this.mMat != this.mDynamicFont.get_material())
				{
					this.mMat.set_mainTexture(this.mDynamicFont.get_material().get_mainTexture());
				}
				return this.mMat;
			}
			if (this.mDynamicFont != null)
			{
				return this.mDynamicFont.get_material();
			}
			return null;
		}
		set
	...
	public bool packedFontShader
	{
		get
		{
			...
			{
				Material material = this.material;
				this.mPacked = ((!(material != null) || !(material.get_shader() != null) || !material.get_shader().get_name().Contains("Packed")) ? 0 : 1);
			}
	...
	public UISpriteData sprite
	{
		get
		{
			...
			if (this.mSprite == null && this.mAtlas != null && !string.IsNullOrEmpty(this.mFont.spriteName))
			{
				...
				{
					this.mSprite = this.mAtlas.GetSprite(base.get_name());
				}
				if (this.mSprite == null)
				{
				...
				while (i < count)
				{
					this.symbols.get_Item(i).MarkAsChanged();
					i++;
				}
			}
			return this.mSprite;
	...
	private Texture dynamicTexture
	{
		get
		{
			...
			if (this.isDynamic)
			{
				return this.mDynamicFont.get_material().get_mainTexture();
			}
			return null;
		}
	}
	...
	private void Trim()
	{
		...
		{
			Rect rect = NGUIMath.ConvertToPixels(this.mUVRect, this.texture.get_width(), this.texture.get_height(), true);
			Rect rect2 = new Rect((float)this.mSprite.x, (float)this.mSprite.y, (float)this.mSprite.width, (float)this.mSprite.height);
			int xMin = Mathf.RoundToInt(rect2.get_xMin() - rect.get_xMin());
			int yMin = Mathf.RoundToInt(rect2.get_yMin() - rect.get_yMin());
			int xMax = Mathf.RoundToInt(rect2.get_xMax() - rect.get_xMin());
			int yMax = Mathf.RoundToInt(rect2.get_yMax() - rect.get_yMin());
	...
	public void MarkAsChanged()
	{
		...
		while (i < num)
		{
			...
			if (uILabel.get_enabled() && NGUITools.GetActive(uILabel.get_gameObject()) && UIFont.CheckIfRelated(this, uILabel.bitmapFont))
			{
		...
		int j = 0;
		int count = this.symbols.get_Count();
		while (j < count)
		{
			this.symbols.get_Item(j).MarkAsChanged();
			j++;
		}
	}
	...
	public bool UsesSprite(string s)
	{
		if (!string.IsNullOrEmpty(s))
		{
			...
			while (i < count)
			{
				BMSymbol bMSymbol = this.symbols.get_Item(i);
