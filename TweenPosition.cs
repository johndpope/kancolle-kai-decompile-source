...
public class TweenPosition : UITweener
{
	...
	public Vector3 value
	{
		get
		{
			return (!this.worldSpace) ? this.cachedTransform.get_localPosition() : this.cachedTransform.get_position();
		}
		set
		{
			if (this.mRect == null || !this.mRect.isAnchored || this.worldSpace)
			{
				if (this.worldSpace)
				{
					this.cachedTransform.set_position(value);
				}
				else
				{
					this.cachedTransform.set_localPosition(value);
				}
			}
			else
			{
				value -= this.cachedTransform.get_localPosition();
