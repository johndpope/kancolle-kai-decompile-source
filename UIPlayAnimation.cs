...
public class UIPlayAnimation : MonoBehaviour
{
	...
	private void Start()
	{
		...
		if (this.target != null && this.target.get_enabled())
		{
			this.target.set_enabled(false);
		}
	}
	...
	{
		if (base.get_enabled() && this.dualState)
		{
			if (UICamera.currentTouch.dragged == base.get_gameObject())
			{
				this.Play(true, true);
			}
	...
	{
		if (base.get_enabled() && this.dualState && UICamera.hoveredObject != base.get_gameObject())
		{
	...
	{
		if (base.get_enabled() && this.trigger == Trigger.OnPress && UICamera.currentTouch.dragged != base.get_gameObject())
		{
			this.Play(false, true);
		}
	}
	...
	public void Play(bool forward, bool onlyIfDifferent)
	{
		if (this.target || this.animator)
		{
			...
			if (this.clearSelection && UICamera.selectedObject == base.get_gameObject())
			{
			...
			if (activeAnimation != null)
			{
				...
				for (int i = 0; i < this.onFinished.get_Count(); i++)
				{
