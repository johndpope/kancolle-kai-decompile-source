...
public class UI2DSprite : UIBasicSprite
{
	...
	public override Texture mainTexture
	{
		get
		{
			...
			{
				return this.mSprite.get_texture();
			}
			if (this.mMat != null)
			{
	...
	public override bool premultipliedAlpha
	{
		get
		{
			if (this.mPMA == -1)
			{
				...
				this.mPMA = ((!(shader != null) || !shader.get_name().Contains("Premultiplied")) ? 0 : 1);
			}
			return this.mPMA == 1;
		}
	}
	...
	public override Vector4 drawingDimensions
	{
		get
		{
			...
			{
				int num5 = Mathf.RoundToInt(this.mSprite.get_rect().get_width());
				int num6 = Mathf.RoundToInt(this.mSprite.get_rect().get_height());
				int num7 = Mathf.RoundToInt(this.mSprite.get_textureRectOffset().x);
				int num8 = Mathf.RoundToInt(this.mSprite.get_textureRectOffset().y);
				int num9 = Mathf.RoundToInt(this.mSprite.get_rect().get_width() - this.mSprite.get_textureRect().get_width() - this.mSprite.get_textureRectOffset().x);
				int num10 = Mathf.RoundToInt(this.mSprite.get_rect().get_height() - this.mSprite.get_textureRect().get_height() - this.mSprite.get_textureRectOffset().y);
				float num11 = 1f;
				float num12 = 1f;
	...
	protected override void OnUpdate()
	{
		...
		if (this.mFixedAspect)
		{
			...
			if (mainTexture != null)
			{
				int num = Mathf.RoundToInt(this.mSprite.get_rect().get_width());
				int num2 = Mathf.RoundToInt(this.mSprite.get_rect().get_height());
				int num3 = Mathf.RoundToInt(this.mSprite.get_textureRectOffset().x);
				int num4 = Mathf.RoundToInt(this.mSprite.get_textureRectOffset().y);
				int num5 = Mathf.RoundToInt(this.mSprite.get_rect().get_width() - this.mSprite.get_textureRect().get_width() - this.mSprite.get_textureRectOffset().x);
				int num6 = Mathf.RoundToInt(this.mSprite.get_rect().get_height() - this.mSprite.get_textureRect().get_height() - this.mSprite.get_textureRectOffset().y);
				num += num3 + num5;
				num2 += num6 + num4;
	...
	public override void MakePixelPerfect()
	{
		...
		{
			Rect rect = this.mSprite.get_rect();
	...
	public override void OnFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		Rect rect = (!(this.mSprite != null)) ? new Rect(0f, 0f, (float)mainTexture.get_width(), (float)mainTexture.get_height()) : this.mSprite.get_textureRect();
		Rect inner = rect;
