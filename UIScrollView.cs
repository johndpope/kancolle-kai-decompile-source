...
public class UIScrollView : MonoBehaviour
{
	...
	public virtual bool shouldMoveHorizontally
	{
		get
		{
			float num = this.bounds.get_size().x;
	...
	public virtual bool shouldMoveVertically
	{
		get
		{
			float num = this.bounds.get_size().y;
			if (this.mPanel.clipping == UIDrawCall.Clipping.SoftClip)
			{
	...
	protected virtual bool shouldMove
	{
		get
		{
			...
			{
				if (bounds.get_min().x < finalClipRegion.x - num)
				{
					return true;
				}
				if (bounds.get_max().x > finalClipRegion.x + num)
				{
					return true;
				}
			}
			...
			{
				if (bounds.get_min().y < finalClipRegion.y - num2)
				{
					return true;
				}
				if (bounds.get_max().y > finalClipRegion.y + num2)
				{
					return true;
				}
			}
	...
	private void Awake()
	{
		...
		if (this.movement != UIScrollView.Movement.Custom && this.scale.get_sqrMagnitude() > 0.001f)
		{
			...
			this.scale = Vector3.get_zero();
		}
		if (this.contentPivot == UIWidget.Pivot.TopLeft && this.relativePositionOnReset != Vector2.get_zero())
		{
			this.contentPivot = NGUIMath.GetPivot(new Vector2(this.relativePositionOnReset.x, 1f - this.relativePositionOnReset.y));
			this.relativePositionOnReset = Vector2.get_zero();
		}
	}
	...
	private void OnEnable()
	{
		...
		if (this.mStarted && Application.get_isPlaying())
		{
	...
	public virtual void UpdateScrollbars(bool recalculateBounds)
	{
		...
		if (this.horizontalScrollBar != null || this.verticalScrollBar != null)
		{
			...
			Bounds bounds = this.bounds;
			Vector2 vector = bounds.get_min();
			Vector2 vector2 = bounds.get_max();
	...
	public virtual void SetDragAmount(float x, float y, bool updateScrollbars)
	{
		...
		this.DisableSpring();
		Bounds bounds = this.bounds;
		if (bounds.get_min().x == bounds.get_max().x || bounds.get_min().y == bounds.get_max().y)
		{
			return;
		}
		Vector4 finalClipRegion = this.mPanel.finalClipRegion;
		...
		float num3 = bounds.get_min().x + num;
		float num4 = bounds.get_max().x - num;
		float num5 = bounds.get_min().y + num2;
		float num6 = bounds.get_max().y - num2;
		if (this.mPanel.clipping == UIDrawCall.Clipping.SoftClip)
		{
	...
	public virtual void MoveRelative(Vector3 relative)
	{
		...
		expr_06.set_localPosition(expr_06.get_localPosition() + relative);
	...
	public void MoveAbsolute(Vector3 absolute)
	{
		...
		Vector3 vector2 = this.mTrans.InverseTransformPoint(Vector3.get_zero());
	...
	public void Press(bool pressed)
	{
		...
		if (this.smoothDragStart && pressed)
		{
			...
			this.mDragStartOffset = Vector2.get_zero();
		}
		if (base.get_enabled() && NGUITools.GetActive(base.get_gameObject()))
		{
			...
			if (pressed)
			{
				...
				this.mPlane = new Plane(this.mTrans.get_rotation() * Vector3.get_back(), this.mLastPos);
				Vector2 clipOffset = this.mPanel.clipOffset;
	...
	public void Drag()
	{
		...
		if (base.get_enabled() && NGUITools.GetActive(base.get_gameObject()) && this.mShouldMove)
		{
			if (this.mDragID == -10)
			{
			...
			if (this.mPlane.Raycast(ray, ref num))
			{
				...
				else if (this.mPanel.CalculateConstrainOffset(this.bounds.get_min(), this.bounds.get_max()).get_magnitude() > 1f)
				{
	...
	{
		if (base.get_enabled() && NGUITools.GetActive(base.get_gameObject()) && this.scrollWheelFactor != 0f)
		{
			this.DisableSpring();
			this.mShouldMove |= this.shouldMove;
	...
	private void LateUpdate()
	{
		...
		if (this.showScrollBars != UIScrollView.ShowCondition.Always && (this.verticalScrollBar || this.horizontalScrollBar))
		{
			bool flag = false;
			bool flag2 = false;
			if (this.showScrollBars != UIScrollView.ShowCondition.WhenDragging || this.mDragID != -10 || this.mMomentum.get_magnitude() > 0.01f)
			{
		...
		if (!this.mPressed)
		{
			if (this.mMomentum.get_magnitude() > 0.0001f || this.mScroll != 0f)
			{
				if (this.movement == UIScrollView.Movement.Horizontal)
				{
