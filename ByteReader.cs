...
public class ByteReader
{
	...
	public static ByteReader Open(string path)
	{
		...
		if (fileStream != null)
		{
			...
			byte[] array = new byte[fileStream.get_Position()];
	...
	{
		return Encoding.get_UTF8().GetString(buffer, start, count);
	}
		...
		while (this.canRead)
		{
			...
			if (!text.StartsWith("//"))
			{
				...
				if (array2.Length == 2)
				{
					...
					dictionary.set_Item(text2, text3);
				}
			}
		}
		return dictionary;
