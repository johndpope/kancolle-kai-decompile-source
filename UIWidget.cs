...
public class UIWidget : UIRect
{
	...
	public UIWidget.Pivot pivot
	{
		...
		set
		{
			if (this.mPivot != value)
			{
				...
				Vector3 vector3 = cachedTransform.get_position();
				float z = cachedTransform.get_localPosition().z;
				vector3.x += vector.x - vector2.x;
				vector3.y += vector.y - vector2.y;
				base.cachedTransform.set_position(vector3);
				vector3 = base.cachedTransform.get_localPosition();
				vector3.x = Mathf.Round(vector3.x);
				vector3.y = Mathf.Round(vector3.y);
				vector3.z = z;
				base.cachedTransform.set_localPosition(vector3);
			}
		}
	}
	...
	public override void Invalidate(bool includeChildren)
	{
		...
		if (this.panel != null)
		{
			...
			this.UpdateVisibility(this.CalculateCumulativeAlpha(Time.get_frameCount()) > 0.001f, visibleByPanel);
			this.UpdateFinalAlpha(Time.get_frameCount());
			if (includeChildren)
			{
	...
	public void ResizeCollider()
	{
		...
		{
			NGUITools.UpdateWidgetCollider(base.get_gameObject());
		}
	}
	...
	public Bounds CalculateBounds(Transform relativeParent)
	{
		...
		{
			Vector3[] localCorners = this.localCorners;
			Bounds result = new Bounds(localCorners[0], Vector3.get_zero());
			for (int i = 1; i < 4; i++)
			{
		...
		Bounds result2 = new Bounds(worldToLocalMatrix.MultiplyPoint3x4(worldCorners[0]), Vector3.get_zero());
		for (int j = 1; j < 4; j++)
		{
	...
	public virtual void MarkAsChanged()
	{
		...
		{
			this.mChanged = true;
			if (this.panel != null && base.get_enabled() && NGUITools.GetActive(base.get_gameObject()) && !this.mPlayMode)
			{
				this.SetDirty();
				this.CheckLayer();
			}
	...
	{
		if (this.mStarted && this.panel == null && base.get_enabled() && NGUITools.GetActive(base.get_gameObject()))
		{
			this.panel = UIPanel.Find(base.cachedTransform, true, base.cachedGameObject.get_layer());
			if (this.panel != null)
			{
	...
	{
		if (this.panel != null && this.panel.get_gameObject().get_layer() != base.get_gameObject().get_layer())
		{
			base.get_gameObject().set_layer(this.panel.get_gameObject().get_layer());
		}
	}
	...
	protected virtual void Awake()
	{
		...
		this.mPlayMode = Application.get_isPlaying();
	}
	...
	protected override void OnInit()
	{
		...
		this.RemoveFromPanel();
		this.mMoved = true;
		if (this.mWidth == 100 && this.mHeight == 100 && base.cachedTransform.get_localScale().get_magnitude() > 8f)
		{
			this.UpgradeFrom265();
	...
	protected virtual void UpgradeFrom265()
	{
		...
		NGUITools.UpdateWidgetCollider(base.get_gameObject(), true);
	}
	...
	protected override void OnAnchor()
	{
		...
		{
			base.cachedTransform.set_localPosition(vector);
			if (this.mIsInFront)
			{
				this.mChanged = true;
			}
	...
	{
		if (!this.mMoved && !this.panel.widgetsAreStatic && base.cachedTransform.get_hasChanged())
		{
			this.mTrans.set_hasChanged(false);
			this.mLocalToPanel = this.panel.worldToLocal * base.cachedTransform.get_localToWorldMatrix();
